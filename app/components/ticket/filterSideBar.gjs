...
export default class FilterSidebarComponent extends Component {
  ...
  <template>
    <div
      class="w-full h-screen bg-white border-r border-gray-200 flex flex-col"
    >
      <div
        class="sticky top-0 bg-white z-10 border-b border-gray-200 px-4 py-3"
      >
      ...
      <div class="flex-1 overflow-y-auto px-4 py-4">
        <div class="space-y-5">
          ...
          <TagInput
            @label="Tags"
            @value={{this.tagsSearch}}
            @onChange={{this.updateTagsSearch}}
          />
          {{! Category }}
          <div class="pb-4">
            <CustomSelect
      ...
      <div class="bg-white border-t border-gray-200 px-4 py-3">
        <button
          type="button"
          class="w-full px-2 py-2 text-sm text-white bg-gray-800 rounded-sm shadow-md hover:bg-gray-700 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-600 focus:ring-offset-2"
        >
          Apply filters
        </button>
      </div>
